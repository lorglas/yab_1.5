...
const rgb_color LightYellow = {255,255,130,255};
const rgb_color Black = {0,0,0,255};

BubbleHelper::BubbleHelper()
{
	stillRunning = true;
	bubble_color = LightYellow;
	txt_color = Black;
...
void BubbleHelper::Helper()
{
    ...
    textwin=new BWindow(BRect(-100,-100,-50,-50),"_TOOLTIPWINDOW",B_BORDERED_WINDOW_LOOK, B_FLOATING_ALL_WINDOW_FEEL,
                B_NOT_MOVABLE|B_AVOID_FOCUS);

    textview=new BTextView(BRect(0,0,50,50),"",BRect(2,2,48,48),B_FOLLOW_ALL_SIDES,B_WILL_DRAW);
    BFont font(be_plain_font);
    // font.SetSize(12);
  	rgb_color text_color= txt_color;
  	textview->SetStylable(true);
    textview->SetFontAndColor(&font,B_FONT_ALL,&text_color);
    textview->MakeEditable(false);
    textview->MakeSelectable(false);
    textview->SetWordWrap(false);
    //textview->SetLowColor(240,240,100);
    textview->SetViewColor(bubble_color);
    //cout << "test" << endl;
...
void BubbleHelper::SetTextColor(rgb_color color)
{
	txt_color = color;
}
